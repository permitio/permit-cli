...
export class ResourceSetGenerator implements HCLGenerator {
  ...
  private detectAdditionalDependencies(conditions: string | ConditionsObject, resourceKey: string): string[] {
    const stringifiedConditions = typeof conditions === 'string' 
      ? conditions 
    ...
    try {
      ...
      const validSets = resourceSets
        ...
        .map(set => {
          ...
          const conditions = (set.conditions as string | ConditionsObject) ?? 
                            ({} as ConditionsObject);
          
          // Start with resource dependency
          const dependencies = [
            `permitio_resource.${resourceKey}`
          ];
          
          // Add any additional detected dependencies
          const additionalDeps = this.detectAdditionalDependencies(conditions, resourceKey);
