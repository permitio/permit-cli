...
export const options = zod.object({
	...
	envId: zod
		...
		.describe(
			option({
				description: 'Pass this if you want to copy to an existing environment',
			}),
		),
	envKey: zod
		.string()
		...
		.describe(
			option({
				description: 'Key for the new environment to copy to',
			}),
		),
	envName: zod
		.string()
...
export default function Copy({
	...
	useEffect(() => {
		...
		const getEnvironmentId = async () => {
			try {
				...
				const { response: scope, error } = await getApiKeyScope(token);
				if (error) {
					setError(
						`Error while trying to get saved environment scope: ${error}`,
					);
					return;
				}
	...
	const form: FormProps = {
		...
		form: {
			title:
				'Please fill either of the forms, Fill [1] if you want to copy to an Existing Environment, else Fill [2] & fill [3] to specify some optional params',
			sections: [
				{
					...
					fields: [
						{
							type: 'string',
							name: 'existingEnvId',
							label: 'ID of the existing Environment',
						},
					],
				},
				{
					...
					fields: [
						{
							type: 'string',
							name: 'newEnvKey',
							label: 'Key for the new Environment (required)',
						},
						{
							type: 'string',
							name: 'newEnvName',
							label: 'Name for the new Environment (required)',
						},
						{
							type: 'string',
							name: 'newEnvDescription',
							label: 'Description for the new Environment',
						},
					],
				},
				{
	...
	return (
		<>
			...
			{authToken && projectFrom && envFrom && state === 'selecting' && (
				<Form {...form} onSubmit={handleEnvCopy} />
			)}
