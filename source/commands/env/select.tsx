...
export const options = zod.object({
	key: zod
		...
		.describe(
			option({
				description:
					'API Key to be used for the environment selection',
			}),
		),
});
...
export default function Select({ options: { key: apiKey } }: Props) {
	...
	const onEnvironmentSelectSuccess = async(_organisation: ActiveState, _project: ActiveState, environment: ActiveState, secret: string) => {
		try{
