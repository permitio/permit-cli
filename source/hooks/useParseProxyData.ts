...
function parseMappingRule(ruleStr: string, index: number): MappingRuleType {
  ...
  if (!parts[0] || !parts[1]) {
    throw new Error(`Mapping rule at index ${index} must include at least "url" and "http_method".`);
  }
 ...
 */
function parseMappingRules(rulesArray: string[]): NonNullable<ProxyConfigOptions["mapping_rules"]> {
