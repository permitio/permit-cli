...
export const generateGraphData = (
    ...
    relationships.forEach((relations, resourceId) => {
        relations.forEach((relation) => {
            if (!existingNodeIds.has(relation.value)) {
                nodes.push({ data: { id: relation.value, label: `Resource: ${relation.value}` } });
                existingNodeIds.add(relation.value);
            }

            edges.push({
                data: { source: resourceId, target: relation.value, label: relation.label },
            });
        });
    });
    ...
    roleAssignments.forEach((assignment) => {
        ...
        if (!existingNodeIds.has(assignment.user)) {
            nodes.push({ data: { id: assignment.user, label: `User: ${assignment.user}` } });
        ...
        if (!existingNodeIds.has(roleNodeId)) {
            nodes.push({ data: { id: roleNodeId, label: `Role: ${assignment.role}` } });
