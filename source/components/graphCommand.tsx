...
export default function Graph({ options }: Props) {
    const { authToken: contextAuthToken, loading: authLoading, error: authError } = useAuth();
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [authToken, setAuthToken] = useState<string | null>(null); // Store resolved authToken
    const [state, setState] = useState<'project' | 'environment' | 'graph'>('project');
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const graphData = generateGraphData(resourcesData, relationsMap, roleAssignmentsData);
    ...
    if (authLoading || loading) {
        return (
            <Text>
                <Spinner type="dots" /> {authLoading ? 'Authenticating...' : 'Loading Permit Graph...'}
            </Text>
        );
    }
