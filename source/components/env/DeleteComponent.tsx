...
export default function DeleteComponent({
  ...
  const [step, setStep] = useState<
    'loading_projects' | 'project_select' | 'loading_environments' | 'environment_select' | 'confirmation' | 'deleting' | 'done' | 'error'
  >('loading_projects');
  ...
  useEffect(() => {
    const fetchEnvironments = async () => {
      ...
      try {
        const { response, error } = await getEnvironments(selectedProject.id, authToken, cookie);
        if (error) {
          setError(`Failed to fetch environments: ${error}`);
          setStep('error');
          return;
        }
        ...
        if (environmentId) {
          const environment = response.find(e => e.id === environmentId);
          if (environment) {
  ...
  }, [authToken, cookie, environmentId, force, getEnvironments, selectedProject, step]);
  ...
  const handleEnvironmentSelect = useCallback((item: SelectItem) => {
  ...
  useEffect(() => {
    const remove = async () => {
      ...
      try {
        ...
        // If the delete succeeded, proceed to success state
        setStep('done');
  ...
  }, [step, selectedProject, selectedEnvironment, authToken, cookie, deleteEnvironment]);

  if (step === 'loading_projects' || step === 'loading_environments') {
    return (
      <Box>
        <Text>
          <Spinner type="dots" /> Loading {step === 'loading_projects' ? 'projects' : 'environments'}...
        </Text>
      </Box>
    );
  }
  ...
  if (step === 'confirmation') {
    return (
      <Box flexDirection="column">
        ...
        <Text color="red">{selectedEnvironment?.name} ({selectedEnvironment?.key})</Text>
        <Text>This action cannot be undone. Type "delete" to confirm:</Text>
        <TextInput value={confirmation} onChange={setConfirmation} onSubmit={handleConfirmationSubmit} />
      </Box>
    );
  }
