...
export default function DeleteComponent({
  ...
  const [step, setState] = useState<'loading_environments' | 'environment_select' | 'confirmation' | 'deleting' | 'done' | 'error'>(
    environmentId ? (force ? 'deleting' : 'confirmation') : 'loading_environments'
  );
  ...
  }, [environmentId, force, getEnvironment, getEnvironments, step]);
  ...
  useEffect(() => {
    const remove = async () => {
      ...
      try {
        const result = await deleteEnvironment(selectedEnvironment.id);
  ...
  }, [deleteEnvironment, selectedEnvironment, step]);
