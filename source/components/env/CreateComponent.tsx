...
export default function CreateComponent({
  ...
  const [step, setState] = useState<'name_input' | 'key_input' | 'description_input' | 'creating' | 'done' | 'error'>(
    initialName ? (initialKey ? 'description_input' : 'key_input') : 'name_input'
  );
  ...
  }, [createEnvironment, key, name]);

  // If initial values are provided, start creating right away
  useEffect(() => {
    if (initialName && initialKey && initialDescription && step === 'description_input') {
      handleDescriptionSubmit(initialDescription);
    }
  }, [handleDescriptionSubmit, initialDescription, initialKey, initialName, step]);
