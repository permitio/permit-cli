...
describe('useGraphDataApi hook', () => {
            ...
            associated_tenants: [
              {
                ...
                resource_instance_roles: [
                  { resource_instance: 'r1', resource: 'Resource', role: 'admin' },
                ],
              },
            ],
          },
  ...
  it('should handle error in resource instances fetch', async () => {
    ...
    const { lastFrame } = render(<TestComponent />);
    await vi.waitFor(() => {
      expect(lastFrame()).toBe('Failed to fetch data. Check network or auth token.');
    });
  });
      ...
      React.useEffect(() => {
        fetchGraphData('proj1', 'env1').then(res => {
          if (res.error) {
            ...
            if (res.data && res.data.nodes.length === 0 && res.data.edges.length === 0) {
