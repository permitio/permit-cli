...
describe('useMigrateRoleAssignments', () => {
  ...
  it('should handle role that does not exist in target environment', async () => {
    ...
    mockGet.mockResolvedValueOnce({
      data: [
        { user: 'user1', role: 'nonexistent', tenant: 'default' }
      ],
      error: null
    });
  ...
  it('should handle conflicts with override strategy', async () => {
    ...
    const { migrateRoleAssignments } = useMigrateRoleAssignments();
    const result = await migrateRoleAssignments('source-env', 'target-env', 'override');
  ...
  it('should handle conflicts with fail strategy', async () => {
    ...
    const { migrateRoleAssignments } = useMigrateRoleAssignments();
    const result = await migrateRoleAssignments('source-env', 'target-env', 'fail');
  ...
  it('should handle API errors', async () => {
    ...
    mockPost.mockResolvedValueOnce({ error: 'Generic API error that does not match any special conditions' });
