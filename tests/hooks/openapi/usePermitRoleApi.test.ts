...
const mockPatchFn = vi.fn().mockResolvedValue({
  data: { key: 'admin', name: 'Administrator', permissions: ['user:read', 'user:write'] },
  error: null,
  response: { status: 200 }
});
...
describe('usePermitRoleApi', () => {
  ...
  it('should list roles', async () => {
    ...
    expect(mockGetFn).toHaveBeenCalledWith('/v2/schema/{proj_id}/{env_id}/roles');
  ...
  it('should get a role by key', async () => {
    ...
    expect(mockGetFn).toHaveBeenCalledWith('/v2/schema/{proj_id}/{env_id}/roles/{role_id}', {
      role_id: 'admin'
    });
  ...
  it('should update a role with permissions', async () => {
    ...
    const updateResult = await updateRole('admin', 'Administrator', ['user:read', 'user:write']);
