...
vi.mock('../../../source/components/AuthProvider', () => ({
  useAuth: vi.fn(() => ({
    ...
    loading: false,
    error: null,
    scope: { organization_id: 'org_1', project_id: 'proj_1', environment_id: 'env_1' }
  }))
}));
...
describe('usePermitUrlMappingApi', () => {
  ...
  it('should create URL mappings', async () => {
    ...
    const mappings = [
      { url: '/api/users', http_method: 'GET', resource: 'user', action: 'read' }
    ];
    
    const createResult = await createUrlMappings(mappings, 'Bearer', 'Authorization');
  ...
  it('should handle create URL mappings error', async () => {
    ...
    const mappings = [
      { url: '/api/users', http_method: 'INVALID', resource: 'user', action: 'read' }
    ];
