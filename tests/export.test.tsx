...
describe('Export Command', () => {
  beforeEach(() => {
    ...
    const mockPermit = {
      api: {
        ...
        resourceAttributes: { list: vi.fn().mockResolvedValue(mockUserAttributes) },
        resourceRelations: { list: vi.fn().mockResolvedValue([]) },
        conditionSets: { list: vi.fn().mockResolvedValue(mockConditionSets) },
        conditionSetRules: { list: vi.fn().mockResolvedValue(mockConditionSetRules) },
      },
    };
  ...
  it('exports configuration successfully to console', async () => {
    ...
    expect(mockValidateApiKeyScope).toHaveBeenCalledWith('test-key', 'environment');
  ...
  it('handles file write errors', async () => {
    ...
    await vi.waitFor(() => {
      expect(lastFrame()).toContain('Failed to export configuration: Permission denied');
    });
