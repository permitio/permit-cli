...
describe('UserSetGenerator', () => {
  ...
  it('properly processes conditions for terraform format', async () => {
    ...
    mockPermit.api.resources.get.mockResolvedValue({
      attributes: {
        ...
        clearance_level: { type: 'number', description: 'User clearance level' }
      }
    });
    ...
    expect(result).toContain('resource "permitio_user_set" "complex_conditions"');
  ...
  it('correctly sets and uses shared user attributes', async () => {
    ...
    mockPermit.api.resources.get.mockRejectedValue(new Error('Should use shared attributes'));
    ...
    expect(result).toContain('resource "permitio_user_set" "attribute_dependent"');
  ...
  it('handles error when fetching user attributes', async () => {
    ...
    mockPermit.api.conditionSets.list.mockResolvedValue(mockConditionSets);
    mockPermit.api.resources.get.mockRejectedValue(new Error('Failed to get user resource'));
