...
describe('Graph component', () => {
  ...
  it('should show message if no graph data is present', async () => {
    (useAuth as any).mockReturnValue({ authToken: 'token', loading: false, error: null });
    (useProjectAPI as any).mockReturnValue({
      getProjects: vi.fn().mockResolvedValue({ data: [{ name: 'Project 1', id: '1' }], error: null }),
    });
    (useEnvironmentApi as any).mockReturnValue({
      getEnvironments: vi.fn().mockResolvedValue({ data: [{ name: 'Env 1', id: '1' }], error: null }),
    });
    (useGraphDataApi as any).mockReturnValue({
      fetchGraphData: vi.fn().mockResolvedValue({ data: { nodes: [] }, error: null }),
    });
    ...
    stdin.write('\u001B[B');
    stdin.write('\r');
    await waitForOutput({ lastFrame }, (out) => out.includes('Select an environment'));
    stdin.write('\u001B[B');
    stdin.write('\r');
    await waitForOutput({ lastFrame }, (out) => out.includes('Environment does not contain any data'));
  ...
  it('should show success message when graph is generated', async () => {
    ...
    (useGraphDataApi as any).mockReturnValue({
      fetchGraphData: vi.fn().mockResolvedValue({ data: { nodes: [{ id: 'node1' }], edges: [] }, error: null }),
    });
    const { lastFrame, stdin } = render(<Graph options={{}} />);
    await waitForOutput({ lastFrame }, (out) => out.includes('Select a project'));
    ...
    expect(lastFrame()).toContain('Graph generated successfully and saved as HTML!');
  });
});
