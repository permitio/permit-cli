...
describe('DeleteComponent', () => {
  ...
  it('skips to environment selection when projectId is provided', async () => {
    const { lastFrame } = render(
      <DeleteComponent environmentId="env1" />
    );
  ...
  it('handles API errors when deleting environment', async () => {
    ...
    expect(lastFrame().toLowerCase().includes('error') || mockExit.mock.calls.length > 0).toBe(true);
  });
  ...
  it('handles empty projects list error', async () => {
    ...
    expect(lastFrame()).toMatch(/Error|No projects|empty|not found|Environment|Select/i);
  });
