		...
		default: () => {
			...
			return {
				...
				migrateAllData: async (sourceEnvId, targetEnvId, options) => {
					...
					try {
						return await originalMigrateAllData(
							sourceEnvId,
							targetEnvId,
							options,
						);
...
describe('useDataMigration', () => {
	...
	it('should migrate all data in correct order', async () => {
		...
		mockMigrateRoleAssignments.mockResolvedValue({
			total: 8,
			success: 8,
			failed: 0,
		});
		...
		expect(mockMigrateResources).toHaveBeenCalledWith(
			'source-env',
			'target-env',
			'override',
		);
		expect(mockMigrateUsers).toHaveBeenCalledWith(
			'source-env',
			'target-env',
			'override',
		);
		expect(mockMigrateRoles).toHaveBeenCalledWith(
			'source-env',
			'target-env',
			'override',
		);
		expect(mockMigrateRoleAssignments).toHaveBeenCalledWith(
			'source-env',
			'target-env',
			'override',
		);
		...
		const roleAssignmentsCallIndex =
			mockMigrateRoleAssignments.mock.invocationCallOrder[0];
	...
	it('should respect conflictStrategy option', async () => {
		...
		const { migrateAllData } = useDataMigration();
		await migrateAllData('source-env', 'target-env', {
	...
	it('should handle errors during migration', async () => {
		...
		mockMigrateResources.mockRejectedValue(
			new Error('Resources migration failed'),
		);

		const { migrateAllData } = useDataMigration();
		await expect(migrateAllData('source-env', 'target-env')).rejects.toThrow(
			'Resources migration failed',
		);
