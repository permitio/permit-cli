...
describe('parseAttributes', () => {
	...
	it('should throw error for invalid format', () => {
		expect(() => parseAttributes('invalid')).toThrow(
			'Invalid attribute format',
		);
		expect(() => parseAttributes('key1:value1,invalid')).toThrow(
			'Invalid attribute format',
		);
	});

	it('should throw error for empty key', () => {
		expect(() => parseAttributes(':value')).toThrow(
			'Attribute key cannot be empty',
		);
		expect(() => parseAttributes('key1:value1,:value2')).toThrow(
			'Attribute key cannot be empty',
		);
	});

	it('should throw error for empty value', () => {
		expect(() => parseAttributes('key:')).toThrow(
			'Value for key "key" cannot be empty',
		);
		expect(() => parseAttributes('key1:value1,key2:')).toThrow(
			'Value for key "key2" cannot be empty',
		);
	});

	it('should handle special characters in values', () => {
		const result = parseAttributes(
			'email:user@example.com,path:/home/user/doc',
		);
		expect(result).toEqual({
